...
docs(
  ...
  render_version  INTEGER NOT NULL DEFAULT 0,       -- render/cache-invalidation counter; bumped on publish / unpublish / delete / restore (F5 cache key). Renamed from `visibility_version` at the Step-5 split (ADR 0040, landed 2026-06-12)
  created_by      TEXT NOT NULL,
  created_at      INTEGER NOT NULL,
  updated_at      INTEGER NOT NULL,
  deleted_at      INTEGER,
...
**ADR 0040 Step 5 reshape — LANDED (2026-06-12).** The live schema (`packages/db/src/schema.ts` → `DocsTable`) now matches the DDL above: the overloaded `visibility` (`workspace | public | private`) is **split** into the orthogonal pair `access_mode ∈ {space, private}` (read scope; not caller-settable — the mode switch is the Step-8 ACL capability) + `published_slug`/`published_at` (publish: minted/stamped by `doc.publish`, cleared by `doc.unpublish` **and** `doc.soft_delete` — a trashed doc leaves the public site, and restore never surprise-republishes). The old `'public'` value is **retired** — "published" is `published_at IS NOT NULL`, independent of `access_mode` (a `private` doc can be published; a `space` doc can be unpublished). Greenfield: clean enum split, **no backfill**. `visibility_version` is renamed `render_version` and bumps on every publish / unpublish / delete / restore. The handlers emit `doc.publish_changed` (renamed from `doc.visibility_changed`) on the outbox, keyed on `render_version` — the seam the future render consumer subscribes to. *Still pending*: the `(public)/[domain]/[slug]` route + outbox→render consumer (need §12 jobs runner, the §7 markdown projection, an anonymous principal), and `doc.list`/`doc.get` still return every non-deleted doc until the Step-6 ceiling resolver gates them.
...
**Never written by capability handlers.** Rebuilt from the Y.Doc on every `onChange` by a debounced projection job (250 ms). The mirror workflow, the FTS indexer, and any read that needs structured block data without parsing CRDT reads here. All writes go through CRDT (ADR 0018). The block-level `visibility` flag is the sole exception — updated synchronously by `block.set_visibility`'s handler inside the dispatcher tx (it is metadata, not CRDT content; it does not need convergent edit semantics — last writer wins), which also bumps the doc's `render_version` so a cached public render re-evaluates which blocks it may show.
...
> **Superseded shape — see [ADR 0040](../adr/0040-tenancy-ia-model.md) (2026-06-01).** Model B replaces the two-table `doc_acls` / `collection_acls` sketch above with **one polymorphic `grants` table** (`resource_kind`, `resource_id`, `subject_kind`, `subject_id`, `role ∈ GRANT_ROLES`, `is_guest`, `workspace_id`), so "who-can-read X" is a single indexed lookup. Enums: `resource_kind ∈ {space, doc}` (`collection` reserved — Collections are pure-navigation), `subject_kind ∈ {user, agent}` (`team` lands with the Teams slice). The most-permissive union (`⊕`) above is replaced by the **ceiling** (mode-select-then-union): a doc's `access_mode ∈ {space, private}` (the de-overloaded successor to `visibility`) selects the baseline term, unioned with explicit + enumerable guest-grants (positive-only roles, no deny rows) — item-scoped guest grants the only widening past a Space's membership. `grants` carries no `deleted_at` (revoke is a hard delete; rows ride with their resource through soft-delete) and, being polymorphic, forgoes the composite `(id, workspace_id)` FK — a fuzzer property compensates by asserting every `resource_id` resolves within its `workspace_id`.
