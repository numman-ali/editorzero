...
> **Superseded shape — see [ADR 0040](../adr/0040-tenancy-ia-model.md) (2026-06-01).** Model B replaces the two-table `doc_acls` / `collection_acls` sketch above with **one polymorphic `grants` table** (`resource_kind`, `resource_id`, `subject_kind`, `subject_id`, `role ∈ GRANT_ROLES`, `is_guest`, `workspace_id`), so "who-can-read X" is a single indexed lookup. v1 enums: `resource_kind ∈ {space, doc}` (`collection` reserved — Collections are pure-navigation in v1), `subject_kind ∈ {user, agent}` (`team` reserved). The most-permissive union (`⊕`) above is replaced by the **ceiling**: `(Space-baseline − narrowing) ∪ enumerable guest-grants`, with item-scoped guest grants the only widening past a Space's membership. `grants` carries no `deleted_at` (revoke is a hard delete; rows ride with their resource through soft-delete) and, being polymorphic, forgoes the composite `(id, workspace_id)` FK — a fuzzer property compensates by asserting every `resource_id` resolves within its `workspace_id`.
