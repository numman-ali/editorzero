...
> **Amended by [ADR 0040](../adr/0040-tenancy-ia-model.md) (2026-06-01).** ADR 0015 originally specified a *three-layer* model (capability + tenant plugin + Postgres RLS). Two changes, reflected affirmatively above: **(1) RLS is no longer a committed layer** (fork #4); the app-layer floor is the design. **(2) the Layer-1 algebra inverts** from the old most-permissive union (`role_default ⊕ workspace_override ⊕ collection_acls ⊕ doc_acls`) to the **ceiling** shown. **Build status (updated 2026-06-12, Step 8 verification LANDED):** the ceiling resolver is LIVE — `loadDocReadResolver` (`packages/capabilities/src/acl/ceiling.ts`, the sole read authority) gates `doc.get`/`doc.list` and all seven doc mutations via the F88 `acl_deny` channel, and `workspaceAwareGate` lands the `acting_as` ∩ delegator intersection (H8) at the composition root. The Step-8 capability families are live (`permission.*`, `space.*` incl. roster + reads, guest family, cross-boundary `doc.move`). The §8.1a fuzzer now runs at BOTH grades: resolver-grade (`packages/capabilities/prop/ceiling-fuzzer.test.ts` — oracle-equality + H6 + privacy, 60 worlds/commit) and the full capability-matrix × dual-driver fuzz (`packages/api-server/prop/tenant-isolation.test.ts` — through the production composition on SQLite AND Postgres, ~1.1k dispatches per driver per run, mutation-tested; ADR 0040 amendment 2026-06-12 records the design). The ACL-audit-replay walk covers every Step-8 verb INCLUDING the `doc.move` crossing branch — the space-collection family slice 1 (ADR 0040 amendment 2026-06-12) gave `collection.create` an optional root-only `space_id` (existence-404 → `assertCanPlaceInSpace` reach; child creates inherit the parent's stored binding behind `assertCanPlaceIn`), so space-bound collections are capability-mintable and the walk exercises legacy→space→legacy crossings under both policies. `collection.move` is fully regime-split (ADR 0040 amendment 2026-06-12, crossing branch): a tagged destination union (`legacy_root` / `space_root` / `collection`), same-bucket re-parents behind placement reach, and cross-bucket moves as audited bulk ACL transitions over the subtree's docs — per-doc `assertCanAdministerDoc` sorted by `doc_id`, destination standing, `acl_policy` rails mirroring `doc.move`, one audit row carrying the full dropped-grant preimages. Both movers + both collection verbs ride the §8.1a fuzz matrix (20 verbs) and the replay walk (collection-level crossings under both policies included). The cross-tenant hard-deny (§8.3) is unchanged and orthogonal.
...
- **Cross-tenant leak fuzzer is a first-class invariant test.** `packages/api-server/prop/tenant-isolation.test.ts` (LANDED 2026-06-12 — placed in `api-server`, not the originally-sketched `packages/db`, because "reachable through any capability call" honestly requires the production composition root: `createApiDispatcher` + `workspaceAwareGate` + the default registry) runs against **both** SQLite and Postgres drivers: for every `(capability, principal_workspace, target_workspace)` combination, assert that no tenant-scoped row from `target_workspace ≠ principal_workspace` is reachable through any capability call — plus one-sided authority oracles, exact `doc.list`/`space.list` visible-set equality, and cross-backend outcome-digest equality (see the ADR 0040 amendment for the property set). Scale: ~1.1k dispatches per driver per run at pre-push (`pnpm test:prop`); the 100k-nightly lane is OPEN until a scheduled CI lane exists — raise `SEEDS` there, the generator is seed-deterministic.
