...
> **Amended by [ADR 0040](../adr/0040-tenancy-ia-model.md) (2026-06-01) — built-vs-specified correction + algebra inversion.** As of today **Layer 3 (RLS) does not exist in code** (zero `CREATE POLICY`; the driver sets no `app.workspace_id` GUC) and the §8.1a cross-tenant fuzzer is **also unbuilt** — so **Layer 2 is the sole tenant-isolation floor on _both_ backends** right now, not just SQLite. Treat the Layer-3 / fuzzer text above and in §8.1a as the target, not the current state. Separately, ADR 0040 **inverts the Layer-1 resolution algebra** for in-Space access: the `(role_default ⊕ workspace_override ⊕ collection_acls ⊕ doc_acls)` most-permissive union becomes a **ceiling** — a doc's `access_mode` (the de-overloaded successor to `visibility`) selects the baseline term (`'space'` = Space members, `'private'` = `created_by`-only), unioned with explicit grants + enumerable guest-grants (positive-only roles, no deny rows) — a local, audit-reconstructable lookup, never a graph walk. Collections hold no ACL. Per ADR 0040 fork #4, **RLS (Layer 3) is not a committed layer** — multi-tenant is a product non-goal; the app-layer floor on both backends is the enforcement design. The cross-tenant hard-deny (§8.3) is unchanged and orthogonal.
