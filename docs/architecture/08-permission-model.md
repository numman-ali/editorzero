...
> **Amended by [ADR 0040](../adr/0040-tenancy-ia-model.md) (2026-06-01).** ADR 0015 originally specified a *three-layer* model (capability + tenant plugin + Postgres RLS). Two changes, reflected affirmatively above: **(1) RLS is no longer a committed layer** (fork #4); the app-layer floor is the design. **(2) the Layer-1 algebra inverts** from the old most-permissive union (`role_default ⊕ workspace_override ⊕ collection_acls ⊕ doc_acls`) to the **ceiling** shown. **Build status (honest):** the ceiling resolver and the §8.1a isolation fuzzer are the committed design but **not yet built** — until they land, Layer 2 alone is the live floor; ADR 0040 sequences the resolver (Step 6) + replay/fuzzer gates before any ACL capability ships. The cross-tenant hard-deny (§8.3) is unchanged and orthogonal.
