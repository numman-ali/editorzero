...
Within a Space, a Doc's read set is bounded by **one of two modes**, selected by the Doc's `visibility` (Collections hold no ACL in v1) — narrowing below the baseline is a **doc-level mode switch, not a per-member subtraction** (resolved fork #5 — Codex HIGH 1):

- **`visibility='space'` (the default — the reinterpreted `'workspace'` value):** the Space baseline applies. Every Space member reads; a Doc grant may only **raise** a member's role (`view`→`comment`/`edit`/`owner`) — it never removes read. This is what "all members see all the Space's docs" means (scenario 3).
- **`visibility='private'`:** the Space baseline does **not** apply. Readers are exactly the owner + holders of an explicit grant + guest grants. This is how a Doc is made more private than its Space (the "restricted doc," as in Notion).

The **only** way to grant a principal who is not a Space member is an explicit **Guest grant** on that item. Therefore:

> **who-can-read(X) = ( visibility(X)='space' ? Space-baseline-members(X.space) : { owner(X) } ) ∪ explicit-grants(X) ∪ guest-grants(X)**
...
1. **Select the baseline term by mode.** `visibility='space'` → the principal's Space-membership role → baseline access (and, reserved, Team-derived grants on the Space). `visibility='private'` → no baseline; the owner is the sole implicit reader.
...
5. **How narrowing-below-baseline is represented (Codex HIGH 1 — the fork the first draft left implicit).** **Resolved: a doc-level mode switch on `docs.visibility` (`'space'` = baseline applies + grants may only raise; `'private'` = allow-list of owner + explicit grants + guests), NOT a per-member deny row on `grants`.** `GRANT_ROLES` stays **positive-only**, so "who-can-read X" is a positive union — audit- and replay-friendly (a deny row would invert the meaning of every union and complicate 3a replay). The one capability this deliberately does **not** support in v1: excluding a single member from an otherwise-`'space'`-visible doc while everyone else keeps reading (this matches Notion, which flips the page to *restricted* rather than per-person deny); the v1 path is `visibility='private'` + an explicit allow-list. **Deny / `effect ∈ {allow,deny}` grants are reserved-future** behind that specific need.
...
- **Side-channel gate (MEDIUM 6):** no read-gating ACL capability ships while **any** export/render path can still enumerate docs *without* the resolver — the event-rendered published path (§5.4) and the git-mirror (ADR 0020) must be ceiling-aware or scope-restricted first (the Step-9 export-scope decision, pulled forward as a precondition). Otherwise the app is correct and the side-channel leaks.
