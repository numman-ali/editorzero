...
> **Amended by [ADR 0040](0040-tenancy-ia-model.md) (2026-06-01) — corrects the record.** Layer 3 is **specified-but-unbuilt**: there are zero `CREATE POLICY` / `ENABLE ROW LEVEL SECURITY` statements in `postgres-ddl.ts` and the driver never sets the `app.workspace_id` GUC. **Layer 2 (the Kysely `TenantScopedDb` plugin) is the sole tenant-isolation floor on _both_ backends today** — Postgres mode currently has the same enforcement floor as SQLite, not a stronger one. (The §8.1a cross-tenant fuzzer cited as the equivalence proof is likewise not yet built.) When RLS lands, it must be co-designed with Model B's guest-grant predicate so Layer 2 and Layer 3 are one definition at two sites. **Additionally**, ADR 0040 inverts the Layer-1 resolution algebra for in-Space access: from the most-permissive union (below) to a **ceiling** — a doc's `visibility` selects the baseline term (`'space'` = Space members, `'private'` = owner-only), unioned with explicit grants + enumerable guest-grants (positive-only roles; no deny rows) — kept a local, audit-reconstructable lookup. The cross-tenant hard-deny in §"Worked examples" is unchanged and orthogonal.
