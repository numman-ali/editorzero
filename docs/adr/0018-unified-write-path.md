...
>
> **Editor substrate change — [ADR 0038](0038-owned-editor-tiptap-direct.md), 2026-05-31.** The editor is now an owned thin block layer over **Tiptap v3**, not BlockNote (ADR 0031 → 0038). The write-path *contract* below is unchanged — dispatcher owns the single DB tx; Hocuspocus provides the per-doc serializer + DB-tx hook; the tx co-commits `doc_updates + outbox(doc.updated) + audit_events + outbox(audit.appended)` (F31). What changes is the **mechanism**: the headless `BlockNoteEditor` bound via `openDirectConnection` is replaced by a **DOM-free, JSON-in** path — `prosemirrorJSONToYDoc` / `updateYFragment` (`@tiptap/y-tiptap`) + `ydoc.transact` inside Hocuspocus `openDirectConnection(name, ctx).transact(fn)`; reads via `@tiptap/static-renderer` + `TiptapTransformer.fromYdoc`. This **removes the `happy-dom` DOM shim from the production write path** (it becomes a test-environment-only concern) and enforces *writes are JSON-in, never HTML-in*. **Caveat to test (Hocuspocus v4 #833):** `DirectConnection.transact` does **not** propagate context into the inner Y transaction, so audit attribution (invariant 3) must be set on the inner `doc.transact(fn, origin)`, not the outer ctx. The owned-editor slice owns the migration of `packages/sync` to this mechanism.
