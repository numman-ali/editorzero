...
| F88 | HIGH | Post-parse deny has no audit path — `PermissionDeniedError` thrown from a handler is rethrown without a deny row; comment claims "handler emits its own deny audit" but handlers have no audit writer | Accepted → Applied | 2026-04-18 (pending commit) |
