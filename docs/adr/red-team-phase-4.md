...
| F91 | HIGH | SQLite driver skips ADR 0007's runtime pragmas (WAL, `foreign_keys`, `synchronous`, busy timeout, etc.) — tests and load work exercise a different SQLite than the architecture sizes | Accepted → Applied | 2026-04-18 (pending commit) |
