...
| F90 | HIGH | `AuditWriteInput` is narrower than the audit schema — no `category`, no `collapsed_count`; `input_hash` computed as 32-bit FNV but architecture says normalized sha256 | Accepted → Applied | 2026-04-18 (pending commit) |
