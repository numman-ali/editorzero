...
- The sign-out arm wraps the `/auth/*` mount (`onAuthRevoked`): `sign-out` closes by session; `revoke-sessions` / `revoke-other-sessions` close by user. **Those three are the supported revocation surfaces.** `POST /auth/revoke-session` (single foreign session, token in body) is mounted but unclosed: it needs a token→session mapping the arm doesn't have, and nothing in the product calls it today. That lookup is REQUIRED before any surface (per-device session UI, an API client) adopts the endpoint — "mounted but unclosed" is a gap held shut by the absence of callers, not a precedent (lift-gate review note, concurred).
- Agent-kind grant subjects close nothing today (no agent sockets exist — see the Decision 4 deferral above). Ordering pinned at the lift-gate review: the registry key shape grows to carry agent identity BEFORE bearer/agent WS enables, not after.
- **Self-review extension (same day): moves joined the revoke-class map.** Decision 5's list missed that a `doc.move` / `collection.move` bucket CROSSING narrows read standing — `adopt_baseline` hard-deletes grant edges, and the BEFORE bucket's placement-derived readers lose access when the AFTER bucket is restrictive. Both derive from the `acl_transition` output echo (whose docstring already calls the dropped-grant preimages "the `permission.revoke` posture"): dropped user-kind grant subjects close always; before-bucket readers close only when the after-space is closed/private (root and open spaces keep every member readable; a missing after-space row reads as restrictive — fail toward closing). Same-bucket moves carry no receipt and close nothing.

**Lift-gate review (Codex, same day) — three reader-set gaps closed before calling the lift complete.** The review's framing held: over-close is acceptable bluntness (re-attach re-runs `collabAuthorize`, the authority); under-close is the leak. All three fixes landed with the lift, plus one story correction:

- **The reader ladder (MUST-FIX 1).** The move extractor's "before-bucket readers" was roster-only — but placement-derived read is a LADDER: the root bucket and open spaces read for every live workspace member (org baseline), personal spaces read for their owner, and user-kind SPACE grants confer read without a roster row. A roster-only close left an open-space org-baseline reader's socket live across a restrictive crossing. `bucketReaders` in the tap now walks the full ladder (missing space row = widest set, fail toward closing); every "readers of bucket X" site uses it. Integration-pinned on the open-space non-roster member case.
- **`space.update` type narrowing joined the map (MUST-FIX 2 — supersedes the named residual above).** The residual's premise was wrong: the tap doesn't need a BEFORE-type echo to avoid rename storms, because `space_type` is only present in the INPUT when the caller is SETTING it — a rename never carries it, and a refusal (personal spaces) never reaches the tap. Input-keyed entry: `space_type` absent or `open` → no close; closed/private → every live member minus the space roster closes (space-grant holders over-close; accepted on a rare admin verb). An output transition echo remains an optional precision refinement, no longer a correctness need.
- **`doc.delete` closes the doc's ROOM, not user sockets (MUST-FIX 3, reshaped).** The gap was real: the tap closed grant-edge holders, but a baseline reader's already-established Hocuspocus subscription (no grant row anywhere) survived the trash. The review proposed closing all live workspace members' sockets; that shape was rejected as storm-bluntness — `doc.delete` is a routine verb, and a workspace-wide socket storm per trash inverts the "no rename-storm" principle the rest of the tap holds. Landed shape: `HocuspocusSync.closeDocumentConnections(doc_id)` sends the per-document revocation Close (4401 reason) to every connection attached to that doc — exactly the affected set, sockets survive for other docs, and no user-level closes at all (grant standing persists through soft-delete; restore revives it; re-attach denies on the trashed row). Verified against pinned Hocuspocus 3.4.4: per-doc `Connection.close` sends a doc-scoped Close message and leaves the socket open.
- **`space.archive` is a hardening-only entry (SHOULD-FIX).** Compliant `space.archive` refuses while live descendants exist, so in the normal path there are no attached readers to strand and the entry closes nobody. It stays, ladder-set, for corrupt-state shapes only (docs stranded under trashed collections) — documented as such rather than removed, because the cost is one no-op query on a rare verb and the corrupt-state close is exactly when you want it.
