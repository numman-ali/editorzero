...
- **Presence stays deferred.** `CollaborationCaret` remains uninstalled per this ADR's own open-bug caveat (#6979/#7232 tables crash, #7213 drift) — a follow-up increment with the smoke the Consequences demand. The provider's default Awareness instance stays ENABLED (3.4.4 ping checks break without at least one awareness-bearing provider per socket; the server's write gate passes Awareness frames). *Issue-tracker re-check 2026-06-13: #7232 (init crash under `HocuspocusProvider`, assigned, unfixed) and #7213 (caret drift) still OPEN; #6979 closed without a named fix version. Deferral stands on current evidence.*
