...
### 1. Typed RPC error contract — one typed envelope, emitted into every route's returns
The dispatcher already returns structured errors; the Web UI must consume them as a **typed envelope**, not by reading status codes ad hoc. We adopt a single **envelope shape** — a discriminated union: success carries the typed payload, failure carries a typed error code + machine-readable detail (the same taxonomy the CLI/MCP surface) — that the ADR 0029 generator emits into **every route's explicit returns** (not via a framework-level global handler, which `hc` cannot see). The `api-client` (ADR 0028) **unwraps this envelope in exactly one place** — call sites get a typed result, never a raw `Response`. The envelope's error arm mirrors the dispatcher's error taxonomy (the mirror rule applied to the *error* contract), so a 409-conflict or permission-denied is the same typed shape on API, CLI, MCP, and UI.

**The error arm must live at the route definition boundary, not in a global handler (Codex finding).** `hc<AppType>` infers each route's response type from the **handler's explicit `c.json(body, status)` returns** — a `describeRoute` response *declaration* documents OpenAPI but does **not** feed `hc` inference, and a Hono global `onError` / error middleware is invisible to it. So the ADR 0029 generator emits each route's typed error arms as **explicit `c.json(envelope, status)` returns in the handler** (likely fed by a dispatcher that returns a discriminated `ok | error` result) — the envelope is part of `AppType`, not bolted on around it.
