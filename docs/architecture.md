...
**Current-state caveat (2026-04-21, `doc.rename` slice — `6077a8f`).** The row-metadata projection that rebuilds `docs.title` / `docs.slug` / `docs.updated_at` from `doc.updated` does not yet exist. Until it lands, `doc.rename` runs a **write-through row-metadata bridge** inside the dispatcher's single write-path tx: the handler UPDATEs all three columns *first* (404 short-circuit if the row is missing or soft-deleted), then opens `ctx.transact` → `setDocTitle(ydoc, title)` from `@editorzero/sync` to rewrite the title block. Both writes land atomically (same SQL tx; throw rolls back the row write, and `BoundSyncService.rollback` evicts the in-memory Y.Doc so the next read rehydrates from committed `doc_updates`). The title-slot rule lives in `setDocTitle`: block 0 heading-1 → `editor.updateBlock` in place (keeps block identity stable); otherwise `editor.insertBlocks` a fresh heading-1 at index 0 with `placement: "before"`. Once the projection lands, the row-side UPDATE disappears and the handler shrinks to just the `ctx.transact` call; the surface contract (input / output / audit effect) does not change.
