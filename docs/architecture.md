...
| 7b | Enumerated **metadata** capabilities (`block.set_visibility, doc.publish, doc.unpublish, doc.delete, doc.restore, doc.move, collection.*`) legally skip `ctx.transact` | **Static + integration** | Planned `@editorzero/arch-lint` whitelist in `transact-called-at-most-once` (zero calls allowed for enumerated set) — F89, not yet implemented; `packages/scopes` `METADATA_ONLY_CAPABILITIES` export is canonical and coherence script diffs it against `architecture.md` §6.5; planned `packages/capabilities/test/metadata-only-set.integration.ts` will assert the runtime behaviour — not yet implemented |
