/**
 * Minimal-app test for `POST /docs/remove_guest/:doc_id`. Owns the
 * route-layer contract (dispatches `doc.remove_guest`, merged
 * param+body edge address, 200 JSON preimage echo, strict body —
 * `role` does not belong on this verb); capability-side semantics
 * (edge dispositions, trash posture) live in the capability's unit
 * test. The 409 `grant_lifecycle_conflict` envelope is pinned in
 * `lib/errors.ts` + the capability test (`toHandlerError` →
 * `{kind: "conflict"}`).
 */
...
function buildApp(dispatch: (invocation: DispatchInvocation) => Promise<unknown>) {
  ...
  app.route("/docs", removeGuest);
  return app;
}

describe("POST /docs/remove_guest/:doc_id", () => {
  it("dispatches doc.remove_guest with the merged edge address, 200 JSON preimage echo", async () => {
    ...
    const res = await app.request(`/docs/remove_guest/${DOC_ID}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ subject_kind: "user", subject_id: SUBJECT_ID }),
    });

    expect(res.status).toBe(200);
    expect(await res.json()).toEqual(output);
    expect(captured?.capability_id).toBe(CapabilityId("doc.remove_guest"));
  ...
  it("rejects a body carrying `role` → 400 before the dispatcher runs (strict keys)", async () => {
