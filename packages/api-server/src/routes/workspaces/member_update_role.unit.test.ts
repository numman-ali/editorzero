/**
 * Minimal-app test for `POST /workspaces/member_update_role`. Owns
 * the route-layer contract (dispatches `workspace.member_update_role`,
 * 200 JSON, zod body, strict keys); capability-side semantics
 * (last-owner invariant, `role_unchanged`, Layer-2 scoping) live in
...
function buildApp(dispatch: (invocation: DispatchInvocation) => Promise<unknown>) {
  ...
  app.openapiRoutes([memberUpdateRole] as const);
  return app;
}
