/**
 * Minimal-app test for `POST /workspaces/member_add`. Owns the
 * route-layer contract (dispatches `workspace.member_add`, 200 JSON,
 * zod body, strict keys); capability-side semantics (revive-in-place,
 * Branch A/B/C, Layer-2 scoping) live in the capability's unit test.
 */
...
describe("POST /workspaces/member_add", () => {
  it("dispatches workspace.member_add with parsed body, 200 JSON", async () => {
    ...
    const res = await app.request("/workspaces/member_add", {
      method: "POST",
    ...
    expect(captured?.capability_id).toBe(CapabilityId("workspace.member_add"));
