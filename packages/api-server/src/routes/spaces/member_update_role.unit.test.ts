/**
 * Minimal-app test for `POST /spaces/member_update_role`. Owns the
 * route-layer contract (dispatches `space.member_update_role`, 200
 * JSON, body validation); capability-side semantics live in the
...
function buildApp(dispatch: (invocation: DispatchInvocation) => Promise<unknown>) {
  ...
  app.route("/spaces", memberUpdateRole);
  return app;
}

describe("POST /spaces/member_update_role/:space_id", () => {
  it("dispatches space.member_update_role with the merged param+body input, 200 JSON", async () => {
    ...
    const res = await app.request(`/spaces/member_update_role/${SPACE_ID}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ user_id: TARGET_ID, role: "owner" }),
    });

    expect(res.status).toBe(200);
    expect(await res.json()).toEqual(output);
    expect(captured?.capability_id).toBe(CapabilityId("space.member_update_role"));
    expect(captured?.input).toEqual({ space_id: SPACE_ID, user_id: TARGET_ID, role: "owner" });
  ...
  it("rejects a malformed space_id → 400 before the dispatcher runs", async () => {
    ...
    const res = await app.request("/spaces/member_update_role/not-a-uuid", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ user_id: TARGET_ID, role: "owner" }),
    });
