...
9. **Codex for peer review + rescue.** Three distinct uses of the `/codex:` skill surface; size of diff is NOT a factor in choosing between them — the *nature of the change* is.
   - **`/codex:review`** — straightforward code review. Run **pre-commit on every change, without exception** (small or large, 1 file or 50). The workflow is: stage the diff → run `/codex:review --scope working-tree` → apply findings → re-stage → commit. Only commit after the review is clean or after every finding is addressed. Like a collaborator reading a diff and leaving comments; apply findings directly, no disposition doc. Never ship a commit that hasn't been through this pass.
   - **`/codex:adversarial-review`** — design-level challenge review. Also pre-commit. Reserved for commits that make non-trivial design choices, introduce architectural primitives, or lock in tradeoffs (new ADR, write-path primitive, schema change, permission rule, capability shape). Like a senior engineer asking "is this the right approach?" — not "is this code correct?". Still apply findings directly; still no disposition doc. Distinct from rule 5's phase-boundary red-team, which produces `docs/adr/red-team-*.md` for BLOCKER-class ADR questions.
   - **`/codex:rescue`** — delegate a substantial coding task when stuck, when a second implementation pass is the faster path, or when iteration is taking long enough that a principal-level peer is the better use of wall-clock. Codex handles large coherent chunks well. **Also use `rescue` to apply remediations** from a `/codex:review` pass: let the primary agent stay focused on architecture work while rescue picks up the small fixes. This delegation posture is appropriate for regular-review remediations; for adversarial-review findings, use judgement — design-level fixes often benefit from staying on the primary agent's reasoning thread, but straightforward adversarial-flagged remediations can still go to rescue.
   Default `--background` on anything that will take codex > 30s. Timing and mode are your judgement, but the pre-commit gate on `/codex:review` (and `/codex:adversarial-review` where it applies) is not optional.
10. **Parallel agents share the working tree; do not isolate.** When another agent (Codex, a second Claude session) is working in the same repo, treat their unstaged / untracked changes the way you would treat any sub-agent's output — let it coexist with yours. Do **not** `git stash`, worktree-carve, or otherwise quarantine their work to build an "isolated" review scope. Stage your own files by path; let `/codex:review --scope working-tree` see everything that is actually in the tree; the findings will be scoped to the diff regardless. The same posture applies in reverse: a parallel agent may freely adjust your in-flight changes (including applying its own review findings to files you staged), and you review the result after.
