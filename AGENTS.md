...
- **Verify library docs at point of use.** Before writing against a pinned dependency (Hocuspocus, Tiptap, Better Auth, Yjs, Kysely, Atlas, MCP SDK, Next.js, Hono), fetch current docs for the pinned version.
...
- **Owned editor write path is DOM-free (ADR 0038; BlockNote + happy-dom evicted 2026-06-12)** — server reads/writes go through `@editorzero/sync`: `readBlocks` (`yXmlFragmentToProseMirrorRootNode` → `pmDocToBlocks`) and `writeBlocks` (`blocksToPmDoc` → `PmNode.fromJSON` + `check()` → `updateYFragment`). No live editor, no `view.dispatch`, no DOM shim anywhere in the trunk. `updateYFragment` (from `@tiptap/y-tiptap`, verified against 3.0.4 source) wraps all reconciliation in ONE `ydoc.transact` → one update event → one `doc_updates` row, and equality-matched children keep their Yjs identity/history — both pinned by unit tests in `packages/sync/src/blocks.unit.test.ts`.
- **`DOC_FRAGMENT = "document-store"`** — the Y.XmlFragment name is part of the durable format (the string predates ADR 0038). Never rename the VALUE without a content migration.
- **Block ids are pre-minted** — `seedBlocks` requires every seed to carry a caller-minted `BlockId` (audit invariant 3a); `readBlocks` throws on a persisted id-less block (it means something wrote the fragment outside the owned path). Browser-created blocks use `id: ""` (sentinel → PM `id: null` attr) until the server mints on insert.
...
- **`@tiptap/*` — exact pins, bump in lockstep** — tiptap packages peer-pin each other *exactly*, so every `@tiptap/*` entry in the `pnpm-workspace.yaml` catalog moves together or installs break. Currently `3.24.0` + `@tiptap/y-tiptap 3.0.4` (the ADR 0038-verified pair); `3.26.1`/`3.0.5` were published 2026-06-11 and sit inside the pnpm `minimumReleaseAge` cooldown. ProseMirror must stay a singleton — after any bump, check the lockfile still resolves ONE `prosemirror-model`.
