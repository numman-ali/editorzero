...
- **BlockNote server-side writes** — `ServerBlockNoteEditor` has no `transact()`. For mutations, open a Hocuspocus direct connection and call `editor.transact()` on `BlockNoteEditor.create({ collaboration: { fragment } })` bound to the live `Y.XmlFragment` (ADR 0018). `blocksToYDoc` loses history.
